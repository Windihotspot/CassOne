<template>
  <MainLayout>
    <div class="clients-page">
      ...
      <div class="grid grid-cols-4 gap-4 mb-6">
        <div v-for="stat in stats" :key="stat.label" class="stat-card">
          <div class="flex items-center justify-between mb-3">
            <v-icon :icon="stat.icon" size="20" :color="stat.iconColor" />
            <span
              class="text-xs font-medium px-1.5 py-0.5 rounded"
              :class="
                stat.trend > 0 ? 'text-emerald-400 bg-emerald-400/10' : 'text-red-400 bg-red-400/10'
              "
            >
      ...
      <!-- Filters & Search -->
      <div class="filter-bar mb-4 flex flex-wrap items-center gap-3">
        <div class="search-wrapper flex-1 min-w-[220px]">
          <v-text-field
            v-model="search"
        ...
        >
          ...
          <template #item.companyName="{ item }">
            <div class="flex items-center gap-3 py-1">
              ...
              <div>
                <div class="text-sm font-medium text-white">{{ item.companyName }}</div>
          ...
          <template #item.regulatoryStanding="{ item }">
            <span class="text-xs text-slate-300">{{ item.regulatoryStanding }}</span>
          </template>
          ...
          <template #bottom="{ pageCount, page, itemsPerPage, setPage }">
            <div class="table-footer flex items-center justify-between px-4 py-3">
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        <div
          ...
          :key="client.id"
          class="client-card cursor-pointer"
        ...
        >
          <div class="flex items-start justify-between mb-4">
            <div class="flex items-center gap-3">
              ...
              <div>
                <div class="text-sm font-semibold text-white">{{ client.companyName }}</div>
                <div class="text-xs text-slate-400">{{ client.registrationNumber }}</div>
              </div>
            </div>
          ...
          <div class="grid grid-cols-2 gap-3 mb-4">
            <div class="info-block">
              ...
              <div class="text-xs text-slate-200 font-medium">{{ client.industry }}</div>
            </div>
            <div class="info-block">
              <div class="text-xs text-slate-500 mb-0.5">Country</div>
              <div class="text-xs text-slate-200 font-medium">{{ client.country }}</div>
            </div>
            <div class="info-block">
              <div class="text-xs text-slate-500 mb-0.5">Risk Level</div>
              <div class="flex items-center gap-1.5">
            ...
            <div class="info-block">
              <div class="text-xs text-slate-500 mb-0.5">Regulatory</div>
      ...
      <v-menu v-model="menuOpen" :target="menuTarget" location="bottom end">
        <v-list class="context-menu" bg-color="#1a2540" density="compact">
          <v-list-item
      ...
      <v-dialog v-model="newClientDialog" max-width="720" persistent scrollable>
        <v-card class="new-client-dialog" bg-color="#0f1729">
          <!-- Dialog Header -->
          <div
          ...
          <div class="px-6 pt-4 pb-2">
            <div class="step-progress flex gap-2">
              <div
                ...
                :class="s <= currentStep ? 'bg-blue-500' : 'bg-slate-700'"
              />
            </div>
          ...
          <v-card-text class="px-6 py-4" style="max-height: 520px; overflow-y: auto">
            ...
            <div v-if="currentStep === 4">
              ...
              <p class="text-xs text-slate-400 mb-4">
                Confirm all details before creating the client profile.
              </p>
              ...
              <div class="review-section mb-4">
                ...
                <div class="grid grid-cols-2 gap-x-6 gap-y-2 mt-2">
                  <div class="review-row">
                    ...
                    ><span class="review-val">{{ form.companyName || '—' }}</span>
                  </div>
                  <div class="review-row">
                    <span class="review-key">Reg. Number</span
                    ><span class="review-val">{{ form.registrationNumber || '—' }}</span>
                  </div>
                  <div class="review-row">
                    <span class="review-key">Country</span
                    ><span class="review-val">{{ form.country || '—' }}</span>
                  </div>
                  <div class="review-row">
                    <span class="review-key">Industry</span
                    ><span class="review-val">{{ form.industry || '—' }}</span>
                  </div>
                </div>
                <div class="review-row mt-2">
                  <span class="review-key">Jurisdictions</span
                  ><span class="review-val">{{ form.jurisdictions.join(', ') || '—' }}</span>
                </div>
              </div>
              ...
              <div class="review-section mb-4">
                ...
                <div class="review-row mt-2">
                  ...
                  ><span class="review-val">{{ form.regulatoryStanding || '—' }}</span>
                </div>
                <div class="review-row mt-2">
                  <span class="review-key">Operations</span
                  ><span class="review-val">{{ form.intendedOperations.join(', ') || '—' }}</span>
                </div>
              </div>
              ...
              <div class="review-section mb-4">
                ...
                <div class="review-row mt-2">
                  ...
                  <span class="flex items-center gap-1.5">
                    ...
                    />
                    <span class="review-val">{{ form.riskLevel || '—' }}</span>
                  </span>
                </div>
                <div class="review-row mt-2">
                  <span class="review-key">Documents</span
                  ><span class="review-val">{{ form.documents.length }} file(s) attached</span>
                </div>
              </div>
          ...
          <!-- Dialog Actions -->
          <div
            class="dialog-footer flex items-center justify-between px-6 py-4 border-t border-slate-700/60"
          >
            <v-btn
              variant="text"
              color="slate"
        ...
        <div v-if="selectedClient" class="h-full flex flex-col">
          ...
          <div class="flex items-center justify-between px-6 py-4 border-b border-slate-700/60">
            <div class="flex items-center gap-3">
              ...
              <div>
                <h3 class="text-base font-semibold text-white">{{ selectedClient.companyName }}</h3>
          ...
          <!-- Status Bar -->
          <div
            class="flex items-center gap-2 px-6 py-3 border-b border-slate-700/40 bg-slate-800/30"
          >
            ...
            <span class="text-slate-600">·</span>
            <span class="text-xs text-slate-400">{{ selectedClient.industry }}</span>
          </div>

          <!-- Drawer Tabs -->
          <v-tabs v-model="drawerTab" class="drawer-tabs px-4 pt-2" density="compact" color="blue">
          ...
          <v-window v-model="drawerTab" class="flex-1 overflow-auto">
            ...
            <v-window-item value="documents" class="px-6 py-4">
              <div class="flex items-center justify-between mb-4">
                <span class="text-xs text-slate-400"
                  >{{ selectedClient.documents?.length || 0 }} documents</span
                >
              ...
              <div v-if="selectedClient.documents?.length" class="space-y-2">
                ...
                >
                  ...
                  <div class="flex-1 min-w-0">
                    <div class="text-xs text-slate-200 truncate">{{ doc.name }}</div>
              ...
              <div class="space-y-3">
                ...
                >
                  ...
                  <div class="flex-1 pb-3">
                    <div class="text-xs text-slate-200">{{ act.action }}</div>
                    <div class="text-[10px] text-slate-500 mt-0.5">{{ act.date }}</div>
                  </div>
                </div>
              </div>
            </v-window-item>
      ...
      <v-dialog v-model="deleteDialog" max-width="420">
        <v-card bg-color="#0f1729" class="pa-2">
          <v-card-title class="text-base text-white pt-4 px-5">Delete Client?</v-card-title>
